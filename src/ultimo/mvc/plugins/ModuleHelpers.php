...
class ModuleHelpers implements ApplicationPlugin, ModulePlugin {
  ...
  public function onModuleCreated(\ultimo\mvc\Module $module) {
    ...
    $module->addPlugin($plugin, 'helper');
  }
  ...
  /**
   * Returns the module helper with the specified qualified name.
   * @param string $qName The qualified name of the module helper.
   * @return ModuleHelper The module helper with the specfied qualified
  ...
  public function getHelper($qName) {
    ...
    if (!array_key_exists($qName, $this->helpers)) {
      ...
      if (!is_subclass_of('\\' . $fqName, '\ultimo\mvc\plugins\ModuleHelper')) {
